...
var MONGODB_URI = 'mongodb://'+process.env.USER+':'+ encodeURIComponent(process.env.PASS) +'@'+process.env.HOST+'.mlab.com:'+process.env.DB_PORT+'/'+process.env.DB;
var collection;
  ...
  return new Promise(function (resolve, reject) {
    if (typeof(key) !== "string") {
      ...
      try {
        ...
        collection.insertOne({"key": key, "value": value},
                             { w: 0, wtimeout: 5 },
                             function (err, res) {
          if (err) {
  ...
  return new Promise(function (resolve, reject) {
    try {
      if (typeof(value) !== "string") {
        reject(new DatastoreKeyNeedToBeStringException(value));
      } else {
        collection.findOne( { $or: [{"key":value},{"value":value}]}, function (err, data) {
          if (err) {
            reject(new DatastoreUnderlyingException(value, err));
          } else {
            try {
              if(data===null){
                ...
                resolve({"original_url": data.key, "short_url":data.value});
              }
            } catch (ex) {
      ...
      reject(new DatastoreUnknownException("get", {"value": value}, ex));
    }
  });
}
